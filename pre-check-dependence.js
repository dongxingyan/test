...
const checkDependenciesHelper = (syncOrAsync, config, callback) => {
    ...
    const isCheckOption = options.checkPackageName && options.checkPackageName.length>0;
    console.log('isCheckOption===',isCheckOption)
    ...
    console.log('options.packageDir===',options.packageDir)
    ...
    if (isCheckOption) {
        options.checkPackageName.forEach(name=>{
            checkPackage({
                name,
                versionString:fullDepsMappings[name],
                isOptional: true
            })
        })
    } else {
